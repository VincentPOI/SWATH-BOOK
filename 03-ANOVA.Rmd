...
```{r, warning=FALSE}
  ...
  p <- ggplot(forplot) + geom_point(aes(x, y , label = id, color = ifelse(y>=-log10(tresh_p) & abs(x)>=tresh_fc, "DE proteins", "not significant")),show.legend = F) +
    ...
    layout(ggplotly(p, tooltip = c("x","y","id")))
```

List of the proteins deemed as differentially expressed : 
```{r}
res <- row.names(resultsANOVA)
saveRDS(res, "resANOVA.rds") #saving the name of the differentially expressed proteins in a R object.
print(res)
